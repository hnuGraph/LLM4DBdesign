...
import asyncio
import os
from agent_chat_physical import main as agent_main
import argparse
import markdown
from xhtml2pdf import pisa
from openai import OpenAI
import subprocess
from test_mermaid import generate_er
import re
CUSTOMER_DIR = 'customer_files'
EXAMPLES_DIR = 'saved_files'


def extract_conceptual_design(text):
    # extract conceptual design as the input of generate_er
...
def run_agent_system(model_name, database_name, requirement_text):   # database_user, database_password, database_port,
    ...
    image_path = generate_er(conceptual_design)
    ...
    """
    return styled_html, html_content, image_path
 

def markdown_to_pdf(db_name, html_content, image_path):
    ...
    try:
        ...
        <body>
            {html_content}
            <h2>ER diagram</h2>
            <img src="{abs_image_path}" alt="ER diagram">
        </body>
        </html>
        """
...
def create_interface():
    ...
    with gr.Blocks(title="SchemaAgent: Auto Relation Database Design System") as interface:
        ...
        submit_btn.click(
            ...
            outputs=[output, original_html, image_path],
        ).then(
            fn=markdown_to_pdf,
            inputs=[database_name, original_html, image_path],
